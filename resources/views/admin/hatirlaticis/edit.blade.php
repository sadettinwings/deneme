...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.hatirlatici.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.hatirlaticis.update", [$hatirlatici->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('talep_sec') ? 'is-invalid' : '' }}" name="talep_sec_id" id="talep_sec_id">
                    @foreach($talep_secs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('talep_sec_id') ? old('talep_sec_id') : $hatirlatici->talep_sec->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('hatirlatma_zamani') ? 'is-invalid' : '' }}" type="text" name="hatirlatma_zamani" id="hatirlatma_zamani" value="{{ old('hatirlatma_zamani', $hatirlatici->hatirlatma_zamani) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('baslik') ? 'is-invalid' : '' }}" type="text" name="baslik" id="baslik" value="{{ old('baslik', $hatirlatici->baslik) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('detay') ? 'is-invalid' : '' }}" name="detay" id="detay">{{ old('detay', $hatirlatici->detay) }}</textarea>
                @if($errors->has('detay'))
                    <div class="invalid-feedback">
                        {{ $errors->first('detay') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ilgili_kullanici') ? 'is-invalid' : '' }}" name="ilgili_kullanici_id" id="ilgili_kullanici_id">
                    @foreach($ilgili_kullanicis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('ilgili_kullanici_id') ? old('ilgili_kullanici_id') : $hatirlatici->ilgili_kullanici->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
