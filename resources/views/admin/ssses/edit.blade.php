...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.sss.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ssses.update", [$sss->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kategori_sec') ? 'is-invalid' : '' }}" name="kategori_sec_id" id="kategori_sec_id">
                    @foreach($kategori_secs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kategori_sec_id') ? old('kategori_sec_id') : $sss->kategori_sec->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('soru') ? 'is-invalid' : '' }}" type="text" name="soru" id="soru" value="{{ old('soru', $sss->soru) }}">
                @if($errors->has('soru'))
                    <div class="invalid-feedback">
                        {{ $errors->first('soru') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('cevap') ? 'is-invalid' : '' }}" name="cevap" id="cevap">{{ old('cevap', $sss->cevap) }}</textarea>
                @if($errors->has('cevap'))
                    <div class="invalid-feedback">
                        {{ $errors->first('cevap') }}
                    </div>
